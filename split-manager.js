class SplitPane {
  ...
  createUI() {
    ...
    splitHBtn.addEventListener("click", () =>
      splitManager.splitPane(this.id, "horizontal"),
    );
    ...
    splitVBtn.addEventListener("click", () =>
      splitManager.splitPane(this.id, "vertical"),
    );
  ...
  renderTabs() {
    ...
    this.tabs.forEach((tab) => {
      ...
      tabEl.className =
        "tab" + (tab.path === this.activeTabPath ? " active" : "");
  ...
  displayFileInEditor(content, _language, viewState) {
    if (!this.editor) {
      this.initEditor();
    }
  ...
  setupResizer(resizer, container, direction) {
    ...
    const onMouseDown = (e) => {
      ...
      const children = Array.from(container.children).filter(
        (c) =>
          c.classList.contains("splitPane") ||
          c.classList.contains("splitContainer"),
      );
      pane1 = children[0];
      pane2 = children[1];
